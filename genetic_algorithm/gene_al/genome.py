...
# Global counter to ensure unique link names
_link_counter = 0
_joint_counter = 0


def get_unique_link_name(base_name):
    """Generate a globally unique link name"""
    global _link_counter
    _link_counter += 1
    return f"{base_name}_link_{_link_counter}"

def get_unique_joint_name(link_name, parent_name):
    """Generate a globally unique joint name"""
    global _joint_counter
    _joint_counter += 1
    return f"{link_name}_to_{parent_name}_joint_{_joint_counter}"

def reset_counters():
    """Reset counters for new creature generation"""
    global _link_counter, _joint_counter
    _link_counter = 0
    _joint_counter = 0
...
class Genome():
    ...
    def expandLinks(parent_link, uniq_parent_name, flat_links, exp_links):
        ...
        for c in children:
            for r in range(int(c.recur)):
                ...
                # Use the new unique naming function
                uniq_name = get_unique_link_name(c.name)
    ...
    def genome_to_links(gdicts):
        # Reset counters for each new creature
        reset_counters()
...
class URDFLink:
    ...
    def to_joint_element(self, adom):
        ...
        # Use unique joint naming function
        joint_name = get_unique_joint_name(self.name, self.parent_name)
        joint_tag.setAttribute("name", joint_name)
